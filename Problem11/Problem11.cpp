...
using namespace std;
int high = 0;
int total (int product)	//checks for higest number
{
	if(product > high)
	{
		high = product;
...
int main()
{
	...
	for(int x = 0;x<20;x++)
	{
		for (int y = 0; y<20;y++)
		{
			...
			if(x < 16 && y < 16)		//down to right
			{
				product = list[x][y]*list[x+1][y+1]*list[x+2][y+2]*list[x+3][y+3];
				highest = total(product);
			}
			if(x < 16 && y > 3)			//up to right
			{
				product = list[x][y]*list[x+1][y-1]*list[x+2][y-2]*list[x+3][y-3];
				highest = total(product);
			}
			if(x > 3 && y < 16)			//down to left
			{
				product = list[x][y]*list[x-1][y+1]*list[x-2][y+2]*list[x-3][y+3];
				highest = total(product);
			}
			if(x > 3 && y > 3)			//up to left
			{
				product = list[x][y]*list[x-1][y-1]*list[x-2][y-2]*list[x-3][y-3];
				highest = total(product);
			}
